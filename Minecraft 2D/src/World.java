...
public class World {
	...
	public Block getBlockByCoords(Point block) {
		...
		int dataX = Math.abs(block.x - (x * CHUNK_WIDTH));
		int dataY = Math.abs(block.y - (y * CHUNK_HEIGHT));
	...
	public void generateTerrain() {
		...
		for (int x = 0; x < world.length; x++) {
			for (int y = 0; y < world[0].length; y++) {
				if (world[x][y] == null) {
					world[x][y] = new Chunk(CHUNK_WIDTH, CHUNK_HEIGHT, this, x, y, y * CHUNK_HEIGHT,  (y * CHUNK_HEIGHT) + CHUNK_HEIGHT);
